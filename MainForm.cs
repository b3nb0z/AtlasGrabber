...
namespace WindowsFormsApplication1
{
    ...
    private static bool ValidateRemoteCertificate(object sender, X509Certificate cert, X509Chain chain, SslPolicyErrors error)
    {
        ...
        private void GetIt2(string fl, int window)
        {
            ...
            Image _Image = null;
            string imageAddress = "https:" + fl;

            Uri address = new Uri("https:" + fl);
        ...
        private void webBrowser1_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            ...
            if (lastclick.AddMilliseconds(300) < DateTime.Now)
            {
                if (opened) // & !uploadInProcess
                {
                    ...
                    if (e.KeyCode == Keys.A || e.KeyCode == Keys.Space) //next gene
                    {
                        //label1.Text = "Loaded " + geneNumber.ToString() + " genes";
                        nextGene();
                    }
                  
                    if (e.KeyCode == Keys.S) //next antibody
                    {
                        this.Enabled = false;
                        Application.DoEvents();
                        this.Enabled = true; 
        ...
        private void download_page(string en)
        {
            ...
            if (comboBox1.SelectedIndex != 0 & comboBox1.Enabled)
            {
                ...
                //File.AppendAllText("URL.txt", myuri1.ToString());
                //webBrowser1.Navigate(myuri1);
                wait_for_browser(webBrowser1, myuri1);
            }
            ...
            if (comboBox2.SelectedIndex != 0 & comboBox2.Enabled)
            {
                ...
                wait_for_browser(webBrowser2, myuri2);
            }
            ...
            if (comboBox3.SelectedIndex != 0 & comboBox3.Enabled)
            {
                ...
                wait_for_browser(webBrowser3, myuri3);
            }
            ...
            if (comboBox4.SelectedIndex != 0 & comboBox4.Enabled)
            {
                ...
                wait_for_browser(webBrowser4, myuri4);
            }
            ...
            if (windowNumber > 0) 
            {
                //File.AppendAllText("pageContent.html", webBrowser1.DocumentText);
        ...
        private void wait_for_browser(WebBrowser wb, Uri uri)
        {
            wb.ScriptErrorsSuppressed = true;
            wb?.Navigate(uri);

            while (wb?.ReadyState == WebBrowserReadyState.Uninitialized || wb?.ReadyState == WebBrowserReadyState.Loading || wb?.ReadyState == WebBrowserReadyState.Loaded)
            {
                Thread.Sleep(10);
                Application.DoEvents();
            }

            string end_of_text = @"</html>";
            while (wb?.DocumentText.IndexOf(end_of_text) == -1)
            {
                Thread.Sleep(10);
                Application.DoEvents();
            }
            ...
            {
                //label11.Text = "Waiting dl...";
                Application.DoEvents();
                Thread.Sleep(200);
                wb?.Refresh();
                wb?.Navigate(uri);
                DateTime timeStart = DateTime.Now;
                TimeSpan waitigTime = TimeSpan.Zero;
                int resetCounter = 0;
                ...
                while (wb?.ReadyState != WebBrowserReadyState.Complete)
                {
                    Thread.Sleep(10);
                    Application.DoEvents();
                    waitigTime = DateTime.Now - timeStart;
                    if (waitigTime >= TimeSpan.FromSeconds(5))
                    {
                        //label11.Text = $"Waiting dl. Req:{resetCounter}.";
                        Application.DoEvents();
                        wb?.Refresh();
                        wb?.Navigate(uri);
                        ...
                        if (resetCounter > 20)
                        {
                            MessageBox.Show("Protein Atlas server is not responding in 20 retries. Please get better connection.");
                            break;
                        }    
                    }
                }
        ...
        private void getImageList(string s1, int window, int abNumber)
        {
            ...
            do
            {
                ...
                searchString += (char)34;
                searchString += @"//images.proteinatlas.org/";
                searchString += ablist[window, abNumber].ToString();
                firstCharacter = s1.IndexOf(searchString, firstCharacter + 4);
                if (firstCharacter != -1)
                {

                    s2 = s1.Substring(firstCharacter + 8, 150);
                   // secondCharacter = s2.IndexOf(as3);
                    ...
                    if (secondCharacter == -1) //full res, not thumbnails
                    {
                        ...
                        if (secondCharacter == -1) //full res, not thumbnails
                        {
                            ...
                            {
                                s3 = s1.Substring(firstCharacter + 8, 540);
                                ...
                                s2 = s2.Substring(0, s2.IndexOf(".jpg") + 4);
                                switch (window)
                                {
                                    case 1:
                    ...
                    while (reader.Read() && j < 100000) // j wil be incremenrted in short run only
                    {
                        switch (reader.NodeType)
                        {
                            case XmlNodeType.Element:
                                ...
                                if (reader.Name == "patient" && tissue.Equals(tissueType, StringComparison.CurrentCultureIgnoreCase))
                                {
